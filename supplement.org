...
1. Definitions of the causal quantities interest and assumptions required for their identification.
2. Outline of the estimation method implemented in =regmedint= and mathematical formulas.
3. Links to software guide for =regmedint= and code to reproduce the example.
4. Visual demonstration of covariate-dependence of effect estimates.

Applied users of the =regmedint= package should benefit most from the sotware instructions in Section 3 and the causal assumptions stated in Section 1. Section 2 is meant for methodologists and developers looking for the implementation details of the =regmedint= package. Section 4 visually demonstrate a somewhat overlooked aspect of the regression-based causal mediation analysis and should assist the users with the appropriate use of the =regmedint= package.\\
...
\begin{align*}
  ...
  \\
  &\text{Marginal effect}\\
  E[PNDE] &= E \left[ E[Y_{a_{1},\uwave{M_{a_{0}}}} | \bC] - E[Y_{a_{0},\uwave{M_{a_{0}}}} | \bC] \right]\\
  ...
  &= \text{ Conditional effect evaluated at } E[\bC]
\end{align*}

The argument is similar for the linear mediator, non-linear outcome case as the extra term does not contain $\bbeta_{2}^{T}\bc$.\\

When there is no treatment-mediator interaction term in the outcome model $(\theta_{3} = 0)$, the term $\bbeta_{2}^{T}\bc$ drops out of the expressions. Only $\theta_{1}(a_{1} - a_{0})$ (treatment coefficient in the outcome model; also agrees with CDE) remains as the direct effect, thus, eliminating the covariate dependence.
...
When there is no treatment-mediator interaction term in the outcome model $(\theta_{3} = 0)$, the term $\bbeta_{2}^{T}\bc$ cancels out of the expressions. Only $\theta_{1}(a_{1} - a_{0})$ (treatment coefficient in the outcome model; also agrees with CDE) remains as the direct effect, thus, eliminating the covariate dependence for PNDE (and also TNDE). However, the TNIE (and also PNIE) expressions remain as non-linear functions of $\bbeta_{2}^{T}\bc$, thus, covariate dependence persists for TNIE.

** Practical implications for reporting
For the cases with a linear /mediator/ model, evaluation at the mean covariate vector gives the effect estimates marginal (population average) interpretations cite:vanderweeleExplanationCausalInference2015 (p37). Thus, reporting these marginal quantities for a well-defined population is useful. Additionally, if there is no treatment-mediator interaction term in the outcome model, uniform conditional effects are implied. \\

For the cases with a logistic /mediator/ model, evaluation at the mean covariate level does not give such marginal interpretations. Thus, care must be taken. It is still possible to report the result evaluated at the mean covariate vector, however, the results should be interpreted as /conditional/ effects for an /average/ individual. If categorical covariates (e.g., region) are involved, an average individual can be ambiguous or non-existent (e.g., individual residing in the average region). Reporting conditional effects evaluated at several meaningful covariate patterns may be preferred.

# Note cite:valeriMediationAnalysisAllowing2013 states "For continuous outcomes, if C were set at its average level we would obtain marginal effects on the entire population." (p139). But this does not seem to apply in general.
